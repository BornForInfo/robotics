...
from simple_parking_maneuver_avd.srv import *
...
class SimpleParkingManeuver(Node):
    ...
    def parking_maneuver(self, request, response):
        ...
        if request.direction == "left":
            ...
            self.driving_maneuver_client.call(DrivingManeuver.Request(direction="backward", steering="right", distance=.35))
            self.driving_maneuver_client.call(DrivingManeuver.Request(direction="backward", steering="straight", distance=.4))
            self.driving_maneuver_client.call(DrivingManeuver.Request(direction="backward", steering="left", distance=.35))
            self.driving_maneuver_client.call(DrivingManeuver.Request(direction="forward", steering="straight", distance=.13))
        else:
